...
def NaiveBayes():
    if len(NameEn.get()) == 0:
        ...
        c = conn.cursor()
        c.execute(
            "CREATE TABLE IF NOT EXISTS NaiveBayes(Name StringVar,Symtom1 StringVar,Symtom2 StringVar,"
            "Symtom3 StringVar,Symtom4 TEXT,Symtom5 TEXT,Disease StringVar)")
...
w2.grid(row=1, column=0, columnspan=2, padx=100, pady=20)
w2 = Label(root, justify=LEFT, text="Contributors: Affan, Habib", fg="Pink", bg="Ivory")
w2.config(font=("Times", 30, "bold italic"))
...
lrLb = Label(root, text="Predicted Disease", fg="white", bg="red", width=20)
lrLb.config(font=("Times", 15, "bold italic"))
lrLb.grid(row=18, column=0, pady=(0, 60), sticky=W)
...
def MakeDecision():
    ...
    NaiveBayes()
    KNN()
    pred4.set(most_frequent([pred1.get(), pred2.get(), pred3.get(), pred4.get()]))
...
ex.grid(row=11, column=1, padx=10)

# Button for predicting the disease using different algorithms
dst = Button(root, text="Predict", command=MakeDecision, bg="Red", fg="yellow")
dst.config(font=("Times", 15, "bold italic"))
dst.grid(row=15, column=1, padx=10, pady=30)



# Showing the output of different algorithms
t1 = Label(root, font=("Times", 16, "bold italic"), text="Predicted Disease", height=1, bg="Light green"
           , width=40, fg="red", textvariable=pred4, relief="sunken").grid(row=18, column=1, padx=10, pady=(0, 60))
